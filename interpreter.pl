...
get_instructions(ProgramCodes, Instructions) :-
...
interpret2(getchar,[],StdIn,L-_-R,L-C-R) :- 
   !,read_line_to_codes(user_input, Line, _Tail),
   Line = [C|StdIn]. 
...
exec(File) :-
   read_file_to_codes(File, ProgramCodes, []),
   exec_codes(ProgramCodes).